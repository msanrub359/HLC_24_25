...
export const validarRegister= [
    ...
    check('role').exists().withMessage('El role es obligatorio'),
    check('role').notEmpty().withMessage('El role no puede estar vacío'),
    check('role').isIn(['admin', 'user', 'guest']).withMessage('El rol debe ser "admin", "user", "guest"'),
    (req, res, next)=>{
        const errores= validationResult(req);
        if (!errores.isEmpty()){ //si hay hay errores
            return res.status(400).json({errors:errores.array()})
