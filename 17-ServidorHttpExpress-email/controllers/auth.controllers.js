...
export const login = async (req, res)=>{
    ...
    try {
        ...
        //generar el token
        const token = jwt.sign({id:result[0].id, role:result[0].role, createTo:new Date().toISOString()},SECRET_KEY,{
            "expiresIn": "1h"
        })
        //generar el resfresh_token (7 días)
        const refreshToken = jwt.sign({id:result[0].id, role:result[0].role, createTo:new Date().toISOString()},REFRESH_SECRET_KEY,{
            "expiresIn": "7d"
        })
        ...
        //enviar un email
        sendMail(email, 'Logueado',
            `<h1>Hola ${result[0].username}</h1><p>Gracias por loguearte en la aplicación</p>`
        )
        //devolver al usuario
...
export const register = async (req, res)=>{
    ...
    try {
        ...
        const [result] =await pool.query("INSERT INTO usuarios (username, password, email, role, is_verif, token_verif) VALUES (?,?,?,?,?,?)", [username, hashPass, email, role, false,activacionToken]);
...
export const autorizarRol=(rolesPermitidos)=>{
    return (req, res, next)=>{
        if (!rolesPermitidos.includes(req.user.role)){
            return res.status(403).json({message:'No tiene permitso para acceder a esta ruta'})
        }
        next();
    }
}
